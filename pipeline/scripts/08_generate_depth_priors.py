...
regularization của repo graphdeco-inria/gaussian-splatting (Hướng đi.md mục 2,
hạng #3 — "Depth Anything V2").
...
dưới):
    ...
    python 08_generate_depth_priors.py --all --split public
    python 08_generate_depth_priors.py --all --split private
...
def main():
    ...
    g.add_argument("--scene", help="1 scene cụ thể, vd HCM0181")
    g.add_argument("--all", action="store_true", help="Toàn bộ scene (dùng cùng --split để lọc)")
    ap.add_argument("--split", choices=["public", "private"], default=None,
                     help="Chỉ dùng cùng --all: lọc theo public_set/private_set1")
    ...
    scenes = [get_scene(args.scene)] if args.scene else all_scenes(args.split)
